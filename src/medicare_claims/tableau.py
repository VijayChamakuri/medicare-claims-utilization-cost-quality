...
def calculated_fields_doc(wb: Workbook) -> str:
    """Every calculated field and parameter in the packaged workbook, written from the same specification."""
    lines = ["# Calculated fields and parameters", "", f"> {BANNER}", "",
             "Generated by `medicare-claims tableau` from the workbook specification in `src/medicare_claims/tableau.py`. "
             "These are the exact formulas inside `tableau/workbook/medicare_claims_bi.twbx`; the QA checklist asks the "
             "tester to confirm them in Tableau.", "", "## Parameters", "", "| Parameter | Type | Values | Default |",
             "|---|---|---|---|"]
    for p in wb.params:
        values = ", ".join(str(m) for m in p.members) if p.members else f"{p.min} to {p.max}"
        lines.append(f"| {p.caption} | {p.datatype} | {values} | {p.value} |")
    lines += ["", "## Calculated fields", "", "| Data source | Field | Formula | Format |", "|---|---|---|---|"]
    for d in wb.datasources:
        for c in d.calcs:
            lines.append(f"| {d.caption} | {c.caption or c.name} | `{c.formula}` | {c.fmt or ''} |")
    lines += ["", "## Filters", "",
