-- GENERATED by scripts/gen_dbt_models.py from sql/07_provider_mart.sql. Do not edit; change sql/ and regenerate.
...
with facility as (
    ...
    select *,
           (peer_count >= {{ var('peer_min_providers') }} and claims >= {{ var('min_claims') }}
            and payment_per_claim_q3 > payment_per_claim_q1
            and payment_per_claim > payment_per_claim_q3 + cast({{ var('iqr_multiplier') }} as double) * (payment_per_claim_q3 - payment_per_claim_q1)
           ) as flag_high_payment_per_claim,
           (peer_count >= {{ var('peer_min_providers') }} and claims >= {{ var('min_claims') }}
            and claims_q3 > claims_q1
            and claims > claims_q3 + cast({{ var('iqr_multiplier') }} as double) * (claims_q3 - claims_q1)) as flag_high_claim_volume
    from stats
)
...
from flagged f
left join {{ ref('dim_provider') }} p on p.provider_type = case when f.provider_type like 'facility%' then 'facility' else f.provider_type end
